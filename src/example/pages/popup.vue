  ...
  <div class="p-20">
    ...
    <div class="my-10">
      ...
      <div class="flex flex-wrap gap-3">
        <button
          type="button"
          class="btn__line--primary-sm"
          @click="openModal('modalShow')"
        >
          Basic
        </button>
        <button
      ...
      <common-modal
        v-model="popup.modalShow.show"
        :title="titleName.modalShow"
        @confirm="handleCancel('modalShow')"
        @cancel="handleCancel('modalShow')"
      >
        <template #content>
          <p>모달 내용</p>
        </template>
      ...
      <!-- 공지사항 상세 팝업 -->
      <common-modal
        v-model="popup.noticeDetail.show"
      ...
      >
        <template #content>
          ...
          2. 안정적인 서비스 제공을 위해 5일 경과 후에는 증속신청이 불가하오니
          사용을 원하는 경우 즉시 사이트를 통해 신청하시기 바랍니다. <br />
          3. 증속서비스는 최대 10일 사용가능하며, 증속에 따른 추가요금은
          발생하지 않습니다. <br />
          4. 증속서비스를 계속 사용원하실 경우 담당 영업사원을 통해 속도변경
          통해 청약접수하시기 바랍니다. <br />
        ...
        <template #footer>
          <button
            ...
            @click="handleCancel('noticeDetail')"
          >
            목록보기
          </button>
        </template>
      ...
      >
        <template #content>
          <p>
            <em>고객번호&#40;가입번호&#41;</em> 50000234567 의 속도가 아래와
            같이 변경됩니다.
          </p>
          <form class="form">
            <FormItem label="다운로드">
